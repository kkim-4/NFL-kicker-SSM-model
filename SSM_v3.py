...
if __name__ == "__main__":
    ...
    n_steps = 30000        
    warmup_steps = 2000    # Shortened KL annealing
    
    # Configure OneCycleLR wrapper for Pyro
    optimizer_config = {
        'optimizer': torch.optim.AdamW, 
        'optim_args': {
            'lr': 0.01, # Initialized here, but OneCycleLR will dynamically override this
            'weight_decay': 1e-4
        },
        'max_lr': 0.01,
        'total_steps': n_steps,
        'pct_start': 0.3, # Spends the first 30% of steps (9,000) ramping up to max_lr
        'div_factor': 25.0,
        'final_div_factor': 10000.0
    }
    
    pyro_scheduler = PyroLRScheduler(torch.optim.lr_scheduler.OneCycleLR, optimizer_config)
